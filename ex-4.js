...
// Start coding here
let totalMembers = bills.filter( bill => bill.member)
totalMembers = totalMembers.map( bill => bill.member.name)
totalMembers = totalMembers.reduce( (bill, name) => {
  if (!bill.includes(name)) {
    bill.push(name)
  }
  return bill
}, [])
